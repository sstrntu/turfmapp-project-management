...
export function* restoreCard(id, listId) {
  yield put(actions.restoreCard(id, listId));

  let card;
  try {
    ({ item: card } = yield call(request, api.restoreCard, id, listId));
  } catch (error) {
    yield put(actions.restoreCard.failure(id, error));
    return;
  }

  yield put(actions.restoreCard.success(card));
}

export function* permanentDeleteCard(id) {
  yield put(actions.permanentDeleteCard(id));

  try {
    yield call(request, api.permanentDeleteCard, id);
  } catch (error) {
    yield put(actions.permanentDeleteCard.failure(id, error));
    return;
  }

  yield put(actions.permanentDeleteCard.success(id));
}
