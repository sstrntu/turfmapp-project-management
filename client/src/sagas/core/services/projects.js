...
export function* restoreProject(id) {
  ...
  let included;
  try {
    ({ item: project, included } = yield call(request, api.restoreProject, id));
  } catch (error) {
    yield put(actions.restoreProject.failure(id, error));
    return;
  }

  const { users, projectManagers, boards, boardMemberships } = included;
  yield put(
    actions.restoreProject.success(project, users, projectManagers, boards, boardMemberships),
  );
}

export function* permanentDeleteProject(id) {
  const { projectId } = yield select(selectors.selectPath);
  ...
  yield put(actions.permanentDeleteProject(id));

  try {
    yield call(request, api.permanentDeleteProject, id);
  } catch (error) {
    yield put(actions.permanentDeleteProject.failure(id, error));
    return;
  }

  yield put(actions.permanentDeleteProject.success(id));
}
