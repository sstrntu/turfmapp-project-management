...
export default function* cardsWatchers() {
  yield all([
    ...
    takeEvery(EntryActionTypes.CARD_RESTORE, ({ payload: { id, listId } }) =>
      services.restoreCard(id, listId),
    ),
    takeEvery(EntryActionTypes.CARD_PERMANENT_DELETE, ({ payload: { id } }) =>
      services.permanentDeleteCard(id),
    ),
