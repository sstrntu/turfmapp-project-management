...
export default function* projectsWatchers() {
  yield all([
    ...
    takeEvery(EntryActionTypes.PROJECT_RESTORE, ({ payload: { id } }) =>
      services.restoreProject(id),
    ),
    takeEvery(EntryActionTypes.PROJECT_PERMANENT_DELETE, ({ payload: { id } }) =>
      services.permanentDeleteProject(id),
    ),
  ]);
}
